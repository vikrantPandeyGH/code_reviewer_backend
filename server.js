...
socket.emit("chat-history", allmessages); });
  ...
  socket.on("code-update", async (code) => { socket.broadcast.to(room).emit("code-update", code); await projectModel.findOneAndUpdate( { _id: room }, { code: code, } ); });
  socket.on("get-code", async () => { const project = await projectModel.findById(room); socket.emit("code-history", project.code || ""); }); socket.on("code-review", async function (code) { const review = await codereviewer(code); socket.emit("code-review", review); }); });
